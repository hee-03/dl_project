...
CLASS_INFO = {
    ...
    1: {"label": "Grade 1 — 경증 (Mild DR)",         "color": "#d7e933", "emoji": "⚠️"},
    2: {"label": "Grade 2 — 중등도 (Moderate DR)",   "color": "#ecc823", "emoji": "🔺"},
    3: {"label": "Grade 3 — 중증 (Severe DR)",       "color": "#eb903b", "emoji": "🟠"},
    4: {"label": "Grade 4 — 증식성 (Proliferative)", "color": "#ff0000", "emoji": "🔴"},
}
...
def make_bbox_only_image(rgb_img: np.ndarray, cam: np.ndarray, threshold: float = 0.70) -> np.ndarray:
    """원본 이미지에 Bounding Box만 표시 (Grad-CAM 히트맵 합성 없음)"""
    h, w = rgb_img.shape[:2]
    result = rgb_img.copy()
...
def make_download_image(img1: np.ndarray, img2: np.ndarray, img3: np.ndarray,
    ...
    """
    matplotlib 없이 PIL로 3장을 가로로 이어붙여 PNG bytes 반환.
    각 이미지 위에 제목 텍스트, 전체 상단에 suptitle 추가.
    """
    H, W = img1.shape[:2]
    TITLE_H  = 30   # 개별 이미지 제목 영역 높이
    SUPER_H  = 40   # 전체 제목 영역 높이
    PAD      = 10   # 이미지 간격
    ...
    for i, (arr, title) in enumerate(zip([img1, img2, img3], titles)):
        ...
        # 이미지 붙이기
        pil_sub = Image.fromarray(arr.astype(np.uint8))
        canvas.paste(pil_sub, (x_off, y_off))
...
def main():
    ...
    # ── 3-패널 시각화 ──────────────────────────────────────────
    # ① 원본  ② Grad-CAM 히트맵  ③ 원본 + Bounding Box만
    st.markdown("---")
    st.markdown("## 🖼️ 시각화")
    ...
    with col1:
        st.image(img_original, caption="① 원본 (전처리 후)", use_container_width=True)
    with col2:
        st.image(img_gradcam, caption="② Grad-CAM 히트맵", use_container_width=True)
    with col3:
        st.image(img_bbox, caption=f"③ 원본 + Bounding Box (임계값 {bbox_threshold:.0%})", use_container_width=True)
    ...
    # ── 결과 이미지 다운로드 (PIL로 합성, matplotlib 불필요) ──────
    st.markdown("---")
